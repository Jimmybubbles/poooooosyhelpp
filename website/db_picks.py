...
def sell_stock(pick_id, sell_price, sell_reason='', sell_image=''):
    ...
    try:
        with conn.cursor() as cur:
            ...
            cur.execute("""
                INSERT INTO jimmy_trades (ticker, action, shares, price, total, pnl, trade_date, sell_reason, sell_image, pick_id)
...
def get_closed_trades():
    """Returns closed US trades with both buy and sell data."""
    conn = get_connection()
    with conn.cursor() as cur:
        cur.execute("""
            SELECT
...
def add_manual_closed_trade(ticker, shares, buy_price, buy_date, buy_reason, buy_image,
    ...
    try:
        with conn.cursor() as cur:
            cur.execute("""
                INSERT INTO jimmy_picks (ticker, shares, buy_price, reason, image_path, bought_date, status)
